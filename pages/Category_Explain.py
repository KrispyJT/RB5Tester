...
# --- Page config ---
st.set_page_config(page_title="Service Category Dashboard", layout="wide")
st.title("🧭 Explore Programs by Service Category")
...
st.markdown("Use the filters below to explore programs by service category, type, and fiscal year.")
...
with st.container():
    ...
    with col1:
        all_services = sorted(df["Service Category"].dropna().unique())
        selected_services = st.multiselect("🔹 Service Category", all_services, default=all_services)

    filtered_programs = df[df["Service Category"].isin(selected_services)]

    with col2:
        all_types = sorted(filtered_programs["Program Type"].dropna().unique())
        selected_types = st.multiselect("🔹 Program Type", all_types, default=all_types)

    with col3:
        fiscal_years = sorted(df["Fiscal Year"].dropna().unique())
        selected_fy = st.radio("🔹 Fiscal Year", options=["All"] + fiscal_years)
...
if df_filtered.empty:
    ...
    for program_type in sorted(df_filtered["Program Type"].unique()):
        with st.expander(f"📘 Program Type: {program_type}", expanded=True):
            df_type = df_filtered[df_filtered["Program Type"] == program_type]
                ...
                for _, row in group_df.iterrows():
                    ...
                    with col1:
                        ...
                        **{metric}**
                        ...
                        """)
                    with col2:
                        color = "#2ca02c" if pct >= 100 else "#ff7f0e" if pct >= 80 else "#d62728"
                        fig = go.Figure(go.Bar(
                            x=[target, actual],
                            y=["Target", "Actual"],
                            orientation='h',
                            marker_color=["#d62728", color]
                        ))
                        fig.update_layout(height=150, margin=dict(l=30, r=30, t=20, b=20))
                        st.plotly_chart(fig, use_container_width=True)
